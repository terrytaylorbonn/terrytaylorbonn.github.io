...
A very compact description of the whole demo could therefore be:
...
#### **TOC**
- **[3c.8_pal_D8_1_demo_goal (0)](/3c.8_pal_D8_1_demo_goal/)**
...
- **[3c.8_pal_D8_7_summary (3)](/3c.8_pal_D8_7_summary/)**

<br>
#### TODO Combine the above with this text
...
This chapter explores Foundry Model Studio and how models fit into the Foundry platform. It looks at the model workflow, how a trained model can subsequently be used for inference, and several ways model inference can be incorporated into Foundry applications and pipelines. The goal is primarily to understand the available Foundry model infrastructure before building the demo.

D8.3 Project (1b)

This chapter sets up the machine-learning project in Foundry that will be used for the rest of the demo. It establishes the project resources and data needed for model development and prepares the environment for the two training approaches that follow. In other words, this is the bridge between examining Foundry's model capabilities and actually building a model.

D8.4 Train in Model Studio (2a)

This chapter trains a machine-learning model using Model Studio, Foundry's higher-level model-development environment. It walks through configuring the training workflow, preparing the data, training the model, and examining the resulting model resources. This demonstrates the more integrated Foundry approach to model training without requiring the entire workflow to be constructed manually in code.

D8.5 Train in Jupyter (2b)

This chapter performs model development using a Jupyter notebook, providing a more code-oriented alternative to Model Studio. The notebook gives direct control over data preparation, feature engineering, train/test splitting, model training, and interaction with Foundry resources. Comparing this chapter with the previous one shows two different interfaces for accomplishing many of the same basic model-development tasks.
...
This chapter summarizes the complete D8 workflow and the Foundry tools used at each stage. It emphasizes that the transform pipeline in this demo was primarily part of the training process, while inference was implemented through the deployed model/function and interactive application. It also points toward the logical next step: connecting the model to a batch or streaming inference pipeline so that new operational data can be evaluated automatically.
