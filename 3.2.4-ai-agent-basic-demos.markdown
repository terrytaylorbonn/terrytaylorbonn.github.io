---
layout: page
title: "3.4 (3.2.4) Agentic TF semantic demos"
...
The TF/UFA semantic capabilities are marked with "@" and are described in [2.5 Agentic LLM (TF/UFA semantic) functionality](/2.4-agentic-llms/). 
...
**TOC**
- **[1b Basic tool (with AI)](/3.4.1b_Basic_tool_with_AI_demo/)**. Demonstrates a simple deterministic tool enhanced with LLM semantic capabilities. Shows how AI can improve a traditional tool without changing the overall control flow.
- **[3 Filesystem](/3.4.3_Filesystem_tool/)**. Demonstrates how an agent can explore, read, and organize files using semantic understanding. Shows the combination of deterministic filesystem operations with LLM-assisted interpretation.
- **[5 RAG](/3.4.5_RAG_retrieval_tool/)**. Demonstrates Retrieval-Augmented Generation (RAG). The agent retrieves relevant documents and uses an LLM to summarize or answer questions based on the retrieved information.
- **[6c MCP (LLM tool choice)](/3.4.6c_MCP_connected_tool_LLM/)**. Demonstrates Model Context Protocol (MCP) style tool selection. The LLM determines which tool should be used while deterministic code performs the actual execution.
...
So yes, make:
- ai_demo_06c_mcp_llm_tool_choice.py -xxx->
